...
GitHub 上叫 `SKILL.md` 的文件已经是**百万量级**，而**到底有多少条，没有人能从外部核到底，我们也不行**。
检索早就不是瓶颈了，**判断才是**。

> 这一段原先写的是「生态已经有 **266 万条**，平均质量分 6.2/12」。两个数都不能要：
> 266 万是 SkillsMP 首页自报的「2,664,116 Collected SKILL.md files」，被我们记成了 skills.sh 的，
> 而且**拿一家目录站的自报库存当了世界**；「平均质量分 6.2/12」在整个仓库里查不到任何出处 ——
> 这个项目**故意没有质量分**（见 `scouts/prep_signals.py`：「这不是质量分，是可核对的事实」）。
> 现在只敢说「百万量级」：`filename:SKILL.md` 报 5,619,712，而 5,619,712 = 5488×1024，
> 三次取样全是 1024 的整数倍 —— 量级站得住，具体数字站不住。

「品味」不做第 N 个目录站 —— 我们比不过任何一家的收录量，也不该比。
我们只做一件事：**从那一大堆数不清的东西里，挑出真正有人在乎、有观点、值得一装的那些，
并且把挑它的理由和它的短处一起写下来。**
