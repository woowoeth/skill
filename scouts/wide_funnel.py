...
回答的不是「扫完了没有」——**扫完不可能**，叫 SKILL.md 的文件是百万量级。回答的是：
...
RED_SCRAPE_HARD = re.compile(
    ...
    r"真人指纹|真实指纹|浏览器指纹\s*\S{0,4}(?:伪装|模拟|绕|过)|(?:不易|不容易|难)被风控|规避风控|"
...
def cmd_coverage(_args) -> None:
    ...
    print("\n分母说明：分母是不可知的 —— filename:SKILL.md 报 561 万但那是 1024 的整数倍（分桶估算），最大的目录站自报 266 万而它的 sitemap 只放得出 1.3 万。本轮枚举到的是上面这些数。"
          "**没有任何一个源可以说「已全量覆盖」。**\n")
