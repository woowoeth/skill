...
{rubric}读下面这份上游 SKILL.md 正文，只回一个 JSON：{{"score": 0-100, "why": "不超过25字"}}
**读者在哪。** 这家店的读者是普通人和创作者。冷要冷在题材上（钓鱼选位、社保断缴、装修漏项、烤肉、单色海报都好），不能冷在读者身上：
企业/开发工具链、他国的法规税务（印度报税、韩国隐私法、意大利立法）、某个产品自家的文档/CI 件，就算做得扎实也只值 40-65，不要给到 80。
**语言本身不是理由**：英文写的生活/创作题材（烧烤、手绘、海报、视频运镜）照样算好货，店主指名的 8 件里有 4 件是英文。
...
# 2026-09-06：把店主 73 件心选里读出来的六个信号 / 五个反信号（docs/TASTE.md）作为可选 rubric。
# 默认关（TASTE_RUBRIC 未设或 0），提示词与 09-03 回测时一字不差；开了就多这一段。
# 为什么做成开关：09-03 回测显示现行提示词在「有用但通用」的难负样本上只有 +11pt，
# rubric 能不能把它拉开是**待测**的，测出来之前不许直接换（taste_backtest.py 的第一条信条）。
TASTE_RUBRIC_TEXT = """店主认可的 73 件有六个信号，判分时逐条找，**每条都要能引出原文一句作证据，引不出来就当没有**：
  ① 肯拦住自己 —— 有具体到这个领域的「绝不/不做/禁止/反模式」，不是通用安全套话
  ② 真干过的人写的 —— 有编不出来的细节、踩过的坑、承认拿不准的地方
  ③ 一条硬约束决定品质 —— 一句能引用的规矩，去掉它整件就塌（如「零生成模型」「数字走 CLI 不口算」）
  ④ 产物是个东西 —— 落下可编辑、可接着用的文件，不是聊完就散
  ⑤ 一种腔调做到底 / 点子够怪 / 冷门但对的人极有用 —— 什么都做的一件也没被认可过
  ⑥ 中文生活现场 —— 同等水平下优先
五个反信号，有一条就不该到 60：第 N 个通用件；同作者套路堆；框架的内部装修；单装做不了完整的事；只能写成「帮你更好地 X」。
星数不是信号：73 件里 22 件不到 50 星。
80 分以上必须至少有 ①②③ 中一条是明确的、且没有反信号。
"""
TASTE_RUBRIC = TASTE_RUBRIC_TEXT if os.environ.get("TASTE_RUBRIC", "0") == "1" else ""
...
def _taste_once(it: dict) -> tuple[int, str]:
    ...
    **一个会把店主指名的货判掉的门槛就是错门槛。**
    ...
    prompt = TASTE_PROMPT.format(body=body[:6000], rubric=TASTE_RUBRIC)
    import urllib.request
    anthropic = LLM_KEY.startswith("sk-ant-") or "anthropic" in LLM_BASE
    if anthropic:
