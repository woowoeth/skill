...
description: >-
  ...
  challenge premises, and expand scope when expansion creates a better product.
  Four modes: SCOPE EXPANSION (dream big), SELECTIVE EXPANSION (hold scope and
  cherry-pick), HOLD SCOPE (maximum rigor), SCOPE REDUCTION (strip to
  essentials). Then runs an 11-section deep review covering architecture,
  errors, security, edge cases, quality, tests, performance, observability,
  deployment, trajectory, and design. Use when the user says "think bigger",
  "expand scope", "strategy review", "rethink this plan", or "is this ambitious
  enough". Suggest it when someone is questioning the scope or ambition of a
  plan. Runs well after office-hours.
allowed-tools:
  - Read
  - Grep
  - Glob
...
2. **Every error has a name.** Don't say "handle errors." Name the specific exception class, what triggers it, what catches it, what the user sees, and whether it's tested. Catch-all error handling (`catch Exception`, `rescue StandardError`, `except Exception`) is a code smell — call it out.
...
* ASCII diagrams in code comments for complex designs — models (state transitions), services (pipelines), controllers (request flow), mixins (behavior), tests (non-obvious setup).
...
**If no design doc exists and the user can't articulate the problem** — they keep changing the problem statement, answer with "I'm not sure," or are clearly exploring rather than reviewing — offer `office-hours`:

> "It sounds like you're still figuring out what to build — that's fine, but that's what `office-hours` is for. Want to run it now? We'll pick up right where we left off."

Options: A) Yes, run `office-hours` now. B) No, keep going.
If they keep going, proceed normally — no guilt, no re-asking. If they choose A, note your current progress so you don't re-ask answered questions, then invoke the `office-hours` skill; afterward, re-run the design doc check and resume.
...
Feed this into the Premise Challenge (0A) and Dream State Mapping (0C). If you find a genuine insight, surface it during the expansion opt-in ceremony as a differentiation opportunity.
...
|---|---|---|
| 11-section deep review, required outputs, review report | `sections/review-sections.md` | Step 0 scope and mode are agreed and you are starting the deep review |
...
Present these approach options via AskUserQuestion. Include the RECOMMENDATION and a `Completeness: N/10` rating on every option — these approaches differ in coverage, so completeness scoring applies.

**STOP.** AskUserQuestion once per issue. Do NOT batch. Recommend + WHY. Do NOT proceed to 0D or 0F until the user responds. A "clearly winning approach" is still an approach decision and still needs explicit user approval before it lands in the plan.
...
### 0D-POST. Persist the CEO Plan (EXPANSION and SELECTIVE EXPANSION only)

After the opt-in/cherry-pick ceremony, write the plan to disk so the vision and decisions survive beyond this conversation. Only run this step for EXPANSION and SELECTIVE EXPANSION modes.

```bash
mkdir -p docs/ceo-plans
```

Write to `docs/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Derive the feature slug from the plan being reviewed (e.g. `user-dashboard`, `auth-refactor`). Use `YYYY-MM-DD` for the date.
...
These represent human-team implementation hours. With an agent doing the implementation, 6 hours of human work compresses to roughly 30-60 minutes. The decisions are identical — the implementation speed is not. Present both scales when discussing effort.

Surface these as questions for the user NOW, not as "figure it out later."

### 0F. Mode Selection

In every mode, the user is 100% in control. No scope is added without explicit approval.

Present four options:

1. **SCOPE EXPANSION:** The plan is good but could be great. Dream big — propose the ambitious version. Every expansion is presented individually for approval.
2. **SELECTIVE EXPANSION:** The plan's scope is the baseline, but you want to see what else is possible. Every expansion opportunity presented individually — cherry-pick the ones worth doing. Neutral recommendations.
...
After the mode is selected, confirm which implementation approach (from 0C-bis) applies under the chosen mode. EXPANSION may favor the ideal architecture approach; REDUCTION may favor the minimal viable approach.

Once selected, commit fully. Do not silently drift.

Present these mode options via AskUserQuestion with a RECOMMENDATION. These options differ in kind (review posture), not coverage — do NOT emit `Completeness: N/10` per option. Note instead: `Options differ in kind, not coverage — no completeness score.`
...
Now read `sections/review-sections.md` and execute every section in it. Do not run the deep review from memory.
