---
name: hit-lab
description: 把内容创作变成可校准的预测循环——打分 → 盲预测 → T+3d 复盘 → 进化 rubric，适用任何能被量化（播放/阅读/收听/点击）的内容；内置一份观点视频 rubric，其他形态可借此起步。触发词："初始化"/"打分这篇"/"启动预测"/"已发布"/"复盘"/"升级 rubric"/"推荐选题"/"抓热点"/"挖问题"/"状态"/"找对标"/"learn from"。首次使用必须先跑 /hit-init，强烈建议先用 /hit-learn-from 导入对标账号作为初始信号源。
...
本文件是**总协议 + 路由器**。具体每个阶段的工作流在 `skills/hit-*/SKILL.md` 各子 skill 里。

## Codex compatibility

Codex 没有 Claude Code 的 slash-command harness。安装到 Codex 后，按自然语言触发同一套路由即可：

- `初始化 hit-lab` → 读取并执行 `skills/hit-init/SKILL.md`
- `打分这篇 scripts/foo.md` → 读取并执行 `skills/hit-score/SKILL.md`
- `启动预测 scripts/foo.md` → 读取并执行 `skills/hit-predict/SKILL.md`
- `拍了 ...` / `已发布 ...` / `复盘 ...` / `升级 rubric` / `状态` → 分别读取对应 `skills/hit-*/SKILL.md`

执行时遵循本文件的三条原则和路由表；不要依赖 `/hit-*` 命令是否存在。Claude Code 专用 hook（`.claude/settings.json`）仍只在 Claude Code 里自动触发；Codex 中需要用户主动说 `状态` 查看 buffer、待复盘和候选池。
...
| 用户说 | 调用 | 前置条件 |
|---|---|---|
| "初始化" / "init" / "首次使用" | `/hit-init` | 无（这是入口） |
| "找对标" / "学这个账号" / "拆这几个对标视频" / "learn from" / "导入对标账号" | `/hit-learn-from` | 已 init；cold-start 强烈建议；后续可随时 --append / --replace |
| "找选题" / "我不知道拍什么" / "seed" / "找前 5 个选题" | `/hit-seed` | 已 init（cold-start 用户专用一次性种子动作） |
| "打分这篇 [path]" / "score this [path]" | `/hit-score` | rubric_notes.md 存在 |
| "启动预测" / "start prediction" / "给这稿子打分并预测" | `/hit-predict` | 已 init + 有最终稿 |
| "拍了 X" / "shot it" / "录完了" | `/hit-shoot` | 对应预测已写（buffer +1） |
| "已发布" / "I shipped it" / "发布链接是 X" | `/hit-publish` | 对应预测文件存在（buffer -1） |
| "复盘" / "retro this" / "T+3d 数据来了" | `/hit-retro` | 对应预测文件存在 + 已过 RETRO_WINDOW_DAYS |
| "构造受众画像" / "更新 persona" / "我的观众是谁" / "build persona" | `/hit-persona` | 已 init；有复盘评论数据（或 benchmark seed） |
| "升级 rubric" / "bump rubric" / "更新公式" | `/hit-bump` | 校准池 ≥ MIN_SAMPLES_FOR_BUMP |
| "推荐选题" / "next topic" | `/hit-recommend` | candidates.md 存在且非空 |
| "抓热点" / "fetch trends" / "今天有什么可做的" / "挖问题"（= 抓真实用户提问） | `/hit-trends` | trend-sources adapter 已配置（日常补充候选池）；"挖问题" 等价于 sources: community-questions |
| "状态" / "status" / "看板" | `/hit-status` | 任意时刻可调 |
| "迁移" / "升级 state" / "schema 版本不对" / "migrate" | `/hit-migrate` | 已 init；用户 git pull 拉了新版后；SessionStart hook 提示 schema mismatch 后 |
...
1. 检查用户当前目录是否有 `.hit-state.json` → 没有 → 强制路由到 `/hit-init`
2. 检查 `predictions/` 下有几个文件含完整 `## 复盘` 段填了真实数据 → 决定 `mode: cold-start | calibration`
3. 把判定结果写回 `.hit-state.json` 后再路由到目标 skill
...
skill 期望用户的项目布局如下。`/hit-init` 会创建缺失项；**绝不在没确认的情况下覆盖**。

```
<user-content-project>/
├── rubric_notes.md                    # 评分规则的真实来源
├── WORKFLOW.md                        # 5 阶段流程文档（hit-init 创建）
├── STATUS.md                          # 看板（hit-status 维护）
├── .hit-state.json                  # 状态文件，子 skill 共享上下文
├── .hit-cache/                      # 不入版本控制
│   ├── usage.jsonl                    # 钩子被动记录的使用日志
│   └── trends-history.jsonl           # hit-trends 的去重缓存
├── .claude/
│   └── settings.json                  # 含 prediction-immutability hook
├── benchmark.md                       # 对标账号信息（hit-learn-from 维护）
├── audience.md                        # 受众画像（hit-persona 派生；blind 硬禁读）
├── scripts/                           # 拍前的所有草稿（hit-seed 写或用户写）
│   └── YYYY-MM-DD_<id>_<short>.md
├── predictions/                       # immutable 预测日志（hook 保护）
│   └── YYYY-MM-DD_<id>_<short>.md     # 与 scripts/ 同 id
├── videos/                            # 拍后才建（hit-shoot 创建）
│   └── YYYY-MM-DD_<id>_<short>/
│       ├── script.md                  # 用户提供的最终拍摄稿（hit-shoot 时询问"和 scripts/ 一致吗"）
│       └── report.md                  # T+3d 抓的数据 + 评论（hit-retro 写）
├── samples/                           # 对标账号视频 / 转录（hit-learn-from 创建）
...
├── README.md                          # 营销门面
├── skills/                            # 子 skill 集
│   ├── hit-init/SKILL.md            # ✅ 入口：onboarding 与脚手架
│   ├── hit-learn-from/SKILL.md      # ✅ 对标账号导入（拆 pattern + 派生 base rubric 信号）
│   ├── hit-seed/SKILL.md            # ✅ Cold-start 选题启动器（brainstorm + 可选 draft）
...
│   ├── hit-recommend/SKILL.md       # ✅ 候选池排序推荐（按 buffer 颜色 + 1 稳 + 1 实验）
│   ├── hit-trends/SKILL.md          # ✅ 热点抓取（日常补充候选池，多 adapter）
│   ├── hit-status/SKILL.md          # ✅ 状态看板（含 buffer 警戒）
│   ├── hit-migrate/SKILL.md         # ✅ schema 升级（老用户 git pull 后用）
│   └── hit-score-blind/SKILL.md     # ✅ Channel B 隔离打分 sub-agent（仅 Task tool 调用）
...
│   ├── state-management.md            # ✅ .hit-state.json 读写约定
...
│   ├── trend-sources/                 # 选题原料源（话题热度 + community-questions 用户提问）
│   └── script-extraction/             # 视频/音频转 script（含 whisper for hit-learn-from）
└── examples/
...
- 修改路由 → 改本文件的"路由表"段
- 子 skill 内部细节 → 直接改对应 `skills/hit-*/SKILL.md`
