...
为小红书 AI 工具实操内容生成稳定、清楚、干净、精致的视频封面。执行模式以用户配置为准；两种模式遵循同一套标题、产物和验收规则。
...
- 用户已经指定封面标题，或要求“封面也用文件名”时，直接锁定原文，只做视觉断行；只有未指定时才按下述方法提炼。视频文件名、封面标题、发布标题分别记录，不能互相覆盖。平台发布标题的长度限制不用于缩短封面标题。
...
- 外置产品 Logo 必须对应当前主标题或用户显式传入的参考资产；字幕里的次要品牌不能替代主产品，没有匹配资产时不放外置 Logo。
