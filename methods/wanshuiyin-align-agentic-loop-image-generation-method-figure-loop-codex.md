...
Codex is the executor; each blind round uses a fresh, vision-capable Codex
subagent. This is independent-context review within the same model family.
Use the shell, file reading and editing, file search, web fetching, image
viewing, local rendering and presentation tools this Codex host actually exposes.
Use native subagent tools for review; no Claude Code or Codex MCP server is
required. Tool names in the examples below refer to this host's interface;
use the equivalent native interface when another Codex host names it differently.
...
Invoke `$method-figure-loop-codex` with `[composition source] → [method to
convey] → p5.js`, plus the project path and any round budget. Take these from
the user's message; no `$ARGUMENTS` substitution is required.

## Constants

- **REVIEWER** — a separate, vision-capable Codex subagent that can open the
  local renders; name the model in `wiki/review-log.md`. Start a fresh agent
  for each blind round with `collaboration.spawn_agent`, setting
  `fork_turns: "none"` and carrying no parent conversation or memory of earlier
  rounds. Send
  the round's second half to that same agent through
  `collaboration.followup_task`, after its first answer arrives; calls are
  serial. Inherit the available model and effort unless the user requests a
  supported alternative. If no such reviewer is available, say so before the
  first blind round, finish the
...
Override in the invocation: `$method-figure-loop-codex "SenseNova-U1 Fig. 3 → our
...
Use `collaboration.spawn_agent` with `fork_turns: "none"` for part 1 and wait
for the actual reading. Then use `collaboration.followup_task` with `target`
set to that agent's returned ID for part 2. Do not include part 2's images or
observations in the first call. A new round always gets a new agent.

```
First call — the reading (collaboration.spawn_agent):
  task_name: figure_review_round_<N>
  fork_turns: "none"
  message: |
    Work read-only in <PROJECT_DIR>; do not modify files. Use your image-viewing
    tool to open the supplied image.
...
Second call — same agent (collaboration.followup_task), after
question 1 is answered:
  target: <reviewer_agent_id returned by the first call>
  message: |
