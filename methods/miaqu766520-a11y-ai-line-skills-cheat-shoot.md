...
4. 提供了的话：算 diff
   ...
   # 直接拷贝安装（非 symlink）时，diff 工具随包在 cheat-shared/tools/
   if [[ -z "${DIFF_TOOL:-}" || ! -f "${DIFF_TOOL:-}" ]]; then
     DIFF_TOOL="$HOME/.claude/skills/cheat-shared/tools/diff_pct.py"
   fi
   ...
   **为什么 normalize + char-level**：line-level diff 在创作者真实场景（draft 是 markdown 长句、拍摄稿是 whisper 转录的口语化短行）算出 ~200% 差异但内容几乎不变。char-level Levenshtein 在 normalize 后稳定反映**内容**差异，而非格式差异。详见 [`tools/diff_pct.py`](../cheat-shared/tools/diff_pct.py) + `tools/diff_pct_test.sh`（3 fixture 在两个 backend 上全过）。
5. **判定 v2 触发**：
...
读完 state 后立即算 buffer + 颜色（按 [cadence-protocol.md](../cheat-shared/references/cadence-protocol.md) 的派生规则）：
