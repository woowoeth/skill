---
name: xiaoou-gzh-layout
description: 微信公众号文章排版引擎，将 Markdown、Word、PDF 或纯文本转换为可直接粘贴到公众号编辑器的 HTML，并按 xiaoou 的排版偏好选择主题、章节编号、关键词标记、引言卡片、目录、代码块、图片/GIF 和署名区。用户提到公众号排版、微信排版、自动排版、一键排版、gzh，或要求生成自定义公众号主题时使用。不用于普通网页、落地页、PPT 或代写文章。
---
...
- `使用 $xiaoou-gzh-layout，把 article.md 排成公众号 HTML，自动推荐主题并完成校验。`
- `用清新绿主题排这篇文章，默认署名 xiaoou。`
- `按这张参考图生成一套新的公众号主题，再用它排版。`
- `把这几套主题各排一版，生成可复制的预览页供我比较。`

默认署名为 `xiaoou`。原文明确署名为其他作者时必须保留原作者，不得冒署。
...
- **题材有明确契合** → 单问确认：「建议用 XX（理由），确认还是换一套？」选项给推荐项 + 1~2 个备选 + "看全部"。题材→主题契合参考 theme-index"适用场景"列：
  - 教程 / 测评 / 清单 / 工具盘点 / 知识整理 / 方法论拆解（信息密度高）→ 清新绿
  - 深度分析 / 观点 / 力量感话题 → 红白色系
  - 设计 / 科技评论 / 专业观点 / 高端品牌 → 石墨极简风
  - 禅意 / 极简生活 / 深度随笔 → 留白禅意风
  - 工具对比 / 创意评测（票据视觉隐喻）→ 票据风
  - 内刊手记 / 深度评测 / 案例复盘 → 橄榄手记
- **无明显倾向** → 默认首选 theme-index 第一行（清新绿）作为推荐项放第一位，用 AskUserQuestion 让用户在几套里选（主题多于 4 套时先按气质分组问，再问该组下主题）。
...
6. **开头引言卡署名**：默认写“—— xiaoou”。如果原文明确署名为其他作者，必须保留原作者，不得冒署 xiaoou；如果文章不需要署名，可以省略。
7. **尾部作者签名区（仅末尾一处）**：默认作者名使用 `xiaoou`。
   - 第一句（作者介绍）：`我是 xiaoou，{{一句话简介}}`。简介只使用用户明确提供或确认的内容；未提供时保留简介占位或只写作者名，不虚构履历。
...
- **不用虚线框**：突出标题/强调用小标签或左竖条（通用库 3a–3e），不要用 `border:…dashed` 四周虚线框包标题。**例外**：主题库明确定义的虚线组件（如清新绿的 quote-box 引用框、oneliner-card 亮点卡）是该主题的风格特征，按主题库用法正常使用。
