...
---
> Adapted for **pi-dev-team** designer mode: prefer the project's existing tokens and UI primitives. Write mockups only via `devteam_mockup` (self-contained HTML). Never edit application code. Persist the token sheet, primitives, and design plan with `devteam_state`.
