...
4. Figures live in a sibling `images/` folder. Use those files as source visuals. If a file is missing, say it was not synced.
...
接受：
- 本地库题名 / DOI / 论文 Markdown 路径（默认）
- 论文同级 `images/` 里的解析图
- Markdown / 文本 / 已有大纲
- 用户提供的参考图或 PPT 截图
- 仅当论文不在本地库时，才用附件 PDF

如果是论文，先从 `.sync/catalog.md` 定位，读 Markdown，再 `read` 同级 `images/` 中被引用的图。目标不是写长文，而是提取适合做 slide 的核心叙事。
...
3. 是否插入真实素材：默认使用本地 `images/` 解析图。是否还要用用户提供的截图？说明预计第几页用哪张图。
