...
4. Language and typography mode — for Chinese mathematical-modeling papers, reports, or competition submissions, load `references/math-modeling-cn-style.md`; default visible labels to Chinese, configure a verified CJK font, and keep the final-size type hierarchy compact. Explicit journal/template language rules or a user request for English override this project default.
5. Modeling evidence mode — for mathematical-modeling papers, classify the figure as quantitative evidence, mathematical model geometry, engineering geometry, critical configuration, validation, or boundary/failure evidence. Prefer deterministic diagrams and real computed results. Do not use generated concept art, decoration, or an unsupported self-evaluation chart as a substitute for a model or numerical result. Remove a panel or figure when it does not change the reader's judgment.
6. Template adaptation — when reusing bundled examples or user-provided plotting code, load `references/asset-adaptation.md` before mapping data or changing the script. Translate visible example labels instead of carrying English demo text into a Chinese paper.
7. Delivery preflight — before final delivery, load `references/qa-contract.md`, run `scripts/validate_figure.py` on the plotting source, then personally inspect the rendered outputs at final size and inside the complete paper page. Judge hierarchy, alignment, whitespace, typography, color harmony, line weight, density, and cross-figure consistency; revise weak aesthetics instead of delivering the first technically valid render. Also inspect float order, figure-to-caption spacing, following long tables/sections, and abnormal blank regions; fix document placement before resizing the plot or adding filler content.
...
The files under `references/` are deep references, not defaults. Open them on demand per the `references.on_demand` table in the manifest — for example `references/figure-contract.md` to build the contract, `references/math-modeling-cn-style.md` for Chinese mathematical-modeling figures, `references/asset-adaptation.md` to reuse a plotting template safely, `references/template-catalog.md` for validated Python CSV templates, `references/api.md` for the Python palette and helpers, `references/r-workflow.md` for R, `references/design-theory.md` for color/typography/export rationale, `references/common-patterns.md` and `references/chart-types.md` for layout/chart recipes, `references/nature-2026-observations.md` for real Nature page archetypes, `references/qa-contract.md` before final delivery, and `references/tutorials.md` / `references/demos.md` for worked examples.
