...
- `close_page` 只能关 t-code 自己 `new_page` 出来的 tab，不要尝试关用户原有的 Gmail / Slack。
...
```bash
ls ~/.tcode/skills-cache/web-access/references/site-patterns/
```
...
**写回机制**：操作中如果发现新站点的陷阱或有效模式，主动写到 `~/.tcode/skills/web-access/references/site-patterns/<domain>.md`（用户级目录，不要写 jar 内置缓存），格式参考已有文件。
...
- 不要替用户输用户名密码：t-code 不做自动登录
- 不要把 references/ 写回 jar 缓存目录：写到 `~/.tcode/skills/web-access/references/site-patterns/` 用户级目录
